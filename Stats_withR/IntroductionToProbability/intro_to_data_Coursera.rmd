...
sfo_feb_flights %>%
  summarise(mean_ad = mean(arr_delay), sd_dd = sd(arr_delay), n = n(), median_ad = median(arr_delay),
            IQR_ad = IQR(arr_delay), max_ad = max(arr_delay), min_ad = min(arr_delay))
```
...
nycflights <-  nycflights %>%
  ...
  mutate(avg_speed = distance / air_time_hrs)
...
nycflights %>%
  group_by(dep_type) %>%
  summarise(n = n(), frac = sum(arr_type == "on time") / n())
